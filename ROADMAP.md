...
| **S1.1b-1b-volume**      | Escrita e leitura em volume       | Uma coleta live persiste ≥100 anúncios e decisões em lotes de 50; `GET listings` devolve todos sem URL crescente ou 414                                  |
| **S1.1b-1b-reliability** | Fechamento e órfão                | A coleta live termina `completed` com contadores; falha pós-coleta não recoleta; lease vencido só termina órfão diante de mensagem reentregue            |
...
[volume](docs/handoffs/HANDOFF-S1.1b-1b-volume.md) ·
[reliability](docs/handoffs/HANDOFF-S1.1b-1b-reliability.md) ·
...
| **S3.2a** | Mediana limpa por segmento ([handoff](docs/handoffs/HANDOFF-S3.2.md)) | Segmento com amostra suficiente devolve mediana aparada por IQR com `n` e janela visíveis; abaixo do mínimo responde `AMOSTRA_INSUFICIENTE` sem extrapolar |
...
| **S3.3a** | Desconto sobre a referência ([handoff](docs/handoffs/HANDOFF-S3.3a.md)) | Anúncio com custo `known` e segmento suficiente recebe o desconto com a conta aberta; faltando qualquer perna, `NAO_RANQUEAVEL` com o motivo. Não é margem: taxas de venda ainda não existem |
...
| **S3.4a** | Lista ordenada e paginação estável ([handoff](docs/handoffs/HANDOFF-S3.4.md)) | Ranqueáveis ordenados pelo desconto sobre a referência; sem lastro em grupo próprio e rotulado, com o motivo por item, nunca ordenado como se fosse pior; paginação sobre snapshot estável |
