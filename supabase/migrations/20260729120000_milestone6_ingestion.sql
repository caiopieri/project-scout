-- Milestone 6: transactional normalized eBay listing ingestion.
CREATE UNIQUE INDEX listing_images_listing_url_unique
  ON public.listing_images (listing_id, url);
CREATE INDEX listings_source_raw_hash_idx
  ON public.listings (source_id, raw_content_hash);
...
BEGIN
  ...
  END IF;
  IF v_created
    OR v_existing.price IS DISTINCT FROM v_price
    OR v_existing.shipping_cost IS DISTINCT FROM v_shipping
    OR v_existing.status IS DISTINCT FROM p_listing->>'status'
  THEN
...
REVOKE ALL ON FUNCTION public.ingest_normalized_ebay_listing(
  UUID, UUID, JSONB, TEXT, TEXT, TEXT
) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.ingest_normalized_ebay_listing(
  UUID, UUID, JSONB, TEXT, TEXT, TEXT
) TO service_role;
