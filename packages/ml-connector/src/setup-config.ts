...
export const mercadoLivreSetupInputSchema = z.object({
  clientId: credentialSchema(256),
  clientSecret: credentialSchema(512),
  accessToken: credentialSchema(4096),
  refreshToken: credentialSchema(4096),
});
export type MercadoLivreSetupInput = z.infer<typeof mercadoLivreSetupInputSchema>;
...
export const updateMercadoLivreDevVars = (current: string, rawInput: MercadoLivreSetupInput) => {
  const input = mercadoLivreSetupInputSchema.parse(rawInput);
  const updates = new Map<string, string>([
    ['ML_CONNECTOR_MODE', 'production'],
    ['ML_CLIENT_ID', input.clientId],
    ['ML_CLIENT_SECRET', input.clientSecret],
    ['ML_ACCESS_TOKEN', input.accessToken],
    ['ML_REFRESH_TOKEN', input.refreshToken],
  ]);
