...
import { createResearchProjectSchema, moneySchema, researchCriteriaSchema } from '@scout/schemas';
...
describe('Milestone 3 deterministic intent', () => {
  it('interprets iPhone storage, broken screen and BRL ceiling', async () => {
    const result = await interpreter.interpret({
      query: 'iPhone 13 128 GB com tela quebrada até R$ 1.800.',
    });
  ...
  it('interprets MacBook memory and USD ceiling', async () => {
    const result = await interpreter.interpret({
      query: 'MacBook Pro 16 com 64 GB e até US$ 2.000.',
    });
  ...
  it('resolves contradictory accepted/rejected defects conservatively', async () => {
    const result = await interpreter.interpret({
      query: 'iPhone 13: aceito bateria ruim, mas não aceito bateria ruim.',
    });
...
describe('Milestone 3 shared schemas', () => {
  ...
  it('rejects empty criteria, unknown defects and contradictory states', () => {
    ...
    expect(
      researchCriteriaSchema.safeParse({ models: ['iPhone 13'], acceptedDefects: ['water_damage'] })
        .success,
    ).toBe(false);
    expect(
      researchCriteriaSchema.safeParse({
        models: ['iPhone 13'],
        acceptedDefects: ['cracked_screen'],
        rejectedDefects: ['cracked_screen'],
      }).success,
    ).toBe(false);
  });
  ...
  it('accepts positive storage and rejects invalid capacities', () => {
    expect(
      researchCriteriaSchema.safeParse({ models: ['iPhone 13'], storageGb: [128] }).success,
    ).toBe(true);
    expect(
      researchCriteriaSchema.safeParse({ models: ['iPhone 13'], storageGb: [0] }).success,
    ).toBe(false);
  });
});
...
describe('Milestone 3 project repository lifecycle and isolation', () => {
  it('creates, lists, edits, archives, restores and soft-deletes a project', async () => {
    ...
    expect(
      (await repository.update(created.id, userA, { name: 'iPhone 13 — revisado' })).name,
    ).toContain('revisado');
  ...
  it('does not allow another user to mutate a project', async () => {
    ...
    const input = createResearchProjectSchema.parse({
      name: 'MacBook reparável',
      naturalLanguageQuery: 'MacBook Pro 16 até USD 2.000.',
      structuredQuery: interpreted.criteria,
      interpretation: { ...interpreted, criteria: undefined },
      status: 'active',
    });
